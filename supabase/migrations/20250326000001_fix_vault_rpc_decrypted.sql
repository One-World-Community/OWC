...
RETURNS TABLE (id uuid, name text, description text, secret text, key_id uuid, nonce text, updated_at timestamptz, created_at timestamptz)
SECURITY DEFINER
AS $$
BEGIN
  RETURN QUERY
  SELECT ds.id, ds.name, ds.description, ds.decrypted_secret, ds.key_id, ds.nonce, ds.updated_at, ds.created_at 
